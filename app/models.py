...
class User(Base):
    ...
    github_username: Mapped[str] = mapped_column(Text, unique=True)
...
class Project(Base):
    ...
    def __repr__(self):
        return (
            f"<Project(project_id={self.project_id}, course='{self.course}', "
            f"project_name='{self.project_name}', project_tag='{self.project_tag}', "
            f"semester_id={self.semester_id}, github_url='{self.github_url}', "
            f"slack_channel='{self.slack_channel}', created_at={self.created_at.isoformat()})>"
        )
...
class UserProject(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<UserProject(project_id={self.project_id}, "
            f"user_id={self.user_id}, "
            f"status_github={self.status_github}, "
            f"status_slack={self.status_slack}, "
...
class IngestUserProjectCSV(Base):
    ...
    project_tag: Mapped[str] = mapped_column(Text)
    first_name: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    last_name: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    email: Mapped[Optional[str]] = mapped_column(Text, nullable=True, default=None, unique=False)
    buid: Mapped[Optional[str]] = mapped_column(Text, nullable=True, default=None, unique=False)
    github_username: Mapped[Optional[str]] = mapped_column(Text, nullable=True, default=None, unique=False)
    outcome: Mapped[Optional[Outcome]] = mapped_column(Enum_(Outcome), nullable=True)
    result: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            name='uq_ingest_user_project_csv_all_columns'
        ),
        Index(
            'ix_ingest_user_project_csv_unique',
            # replace NULLs with empty strings to ensure uniqueness on comparison
            func.coalesce('project_name', ''),
            func.coalesce('project_tag', ''),
            func.coalesce('first_name', ''),
            func.coalesce('last_name', ''),
            func.coalesce('email', ''),
            func.coalesce('buid', ''),
            func.coalesce('github_username', ''),
            unique=True
        ),
    )
