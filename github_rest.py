...
def check_user_exists(user: str) -> Tuple[int, Optional[str]]:
    """
    ...
    response = requests.get(
        f'https://api.github.com/users/{user}', headers=headers, timeout=2)
...
def generate_headers() -> dict:
    """
    Generates the headers required for making requests to the GitHub API.

    Returns:
        dict: A dictionary containing the headers with 
        the Accept, Authorization (with the GitHub Personal Access Token), and X-GitHub-Api-Version fields.
    """
...
def add_user_to_project(ssh_url: str, user: str, permission: Literal['pull', 'triage', 'push', 'maintain', 'admin']) -> Tuple[int, Optional[str]]:
    """
    ...
    try:
        ...
        username, repo_name = extract_user_repo_from_ssh(ssh_url)

        status_code, error_message = check_user_exists(user)
        if error_message:
...
def remote_from_from_project(ssh_url: str, user: str) -> Tuple[int, Optional[str]]:
    """
    ...
    try:
        ...
        permissions_response = requests.get(
            f'https://api.github.com/repos/{username}/{repo_name}/collaborators/{user}/permission', headers=generate_headers(), timeout=2)
        ...
        remove_response = requests.delete(
            f'https://api.github.com/repos/{username}/{repo_name}/collaborators/{user}', headers=generate_headers(), timeout=2)
...
def get_users_on_repo(ssh_url: str) -> set[str]:
    """
...
def remove_all_users_from_repo(ssh_url: str) -> list[tuple[int, str]]:
    """
    ...
    """
    try:
        collaborators = get_users_on_repo(ssh_url)
    except Exception as e:
        return -1, str(e)
...
def set_repo_users(ssh_url: str, desired_users: set[str]) -> list[tuple[int, str]]:
    """
    ...
    try:
        current_users = get_users_on_repo(ssh_url)
    except Exception as e:
        return [(-1, str(e))]
...
def main():
    ...
    csv_file_path = '519.csv'
    # Define the path to the output log file
    log_file_path = '519_output.txt'
    ...
    with open(log_file_path, 'a') as log_file:
        ...
        with open(csv_file_path, newline='') as csvfile:
            ...
            for repo_ssh_url, users in repo_users_map.items():
                ...
                try:
                    result = set_repo_users(repo_ssh_url, users)
